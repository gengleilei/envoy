...
public:
  ...
  Network::ReadFilterCallbacks* read_callbacks() const { return read_callbacks_; }

  size_t active_message_list_size() const { return active_message_list_.size(); }
...
private:
  ...
  void sendLocalReply(MessageMetadata& metadata, const DubboFilters::DirectResponse& response,
                      bool end_stream);
  void deferredMessage(ActiveMessage& rpc);
