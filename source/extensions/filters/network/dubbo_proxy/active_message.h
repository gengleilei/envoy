...
public:
  ResponseDecoder(ActiveMessage& parent, Deserializer& deserializer, Protocol& protocol);
  ~ResponseDecoder();
...
class ActiveMessageDecoderFilter : public DubboFilters::DecoderFilterCallbacks,
                                   LinkedObject<ActiveMessageDecoderFilter> {
public:
  ...
  DeserializerType downstreamDeserializerType() const override;
...
public:
  ...
  Network::FilterStatus messageBegin(MessageType type, int64_t message_id,
                                     SerializationType serialization_type) override;
  Network::FilterStatus messageEnd(MessageMetadataSharedPtr metadata) override;
  Network::FilterStatus transferHeaderTo(const Buffer::Instance& header_buf, size_t size) override;
  Network::FilterStatus transferBodyTo(const Buffer::Instance& body_buf, size_t size) override;
  ...
  ConnectionManager& connection_manager() const { return parent_; }
...
private:
  ...
  friend class ActiveMessageDecoderFilter;
  friend class ResponseDecoder;
};
