...
Network::FilterStatus Router::transportEnd() {
  upstream_request_->conn_data_->connection().write(upstream_request_buffer_, false);
  return Network::FilterStatus::Continue;
}

Network::FilterStatus Router::messageBegin(MessageType type, int64_t message_id,
...
Network::FilterStatus Router::messageEnd(MessageMetadataSharedPtr metadata) {
  ...
  upstream_request_.reset(new UpstreamRequest(*this, *conn_pool, metadata,
                                              callbacks_->downstreamDeserializerType(),
...
Router::UpstreamRequest::UpstreamRequest(Router& parent, Tcp::ConnectionPool::Instance& pool,
                                         ...
                                         DeserializerType deserializer_type,
                                         ProtocolType protocol_type)
    : parent_(parent), conn_pool_(pool), metadata_(metadata),
      deserializer_(
          NamedDeserializerConfigFactory::getFactory(deserializer_type).createDeserializer()),
...
void Router::UpstreamRequest::onRequestStart(bool continue_decoding) {
  ...
  ENVOY_LOG(debug, "dubbo upstream request: downstream size {}",
