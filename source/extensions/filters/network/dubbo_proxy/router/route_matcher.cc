...
bool ParamterRouteEntryImpl::matchParameter(const MessageMetadata::ParameterValue& request_data,
                                            const ParamterData& config_data) const {
  switch (config_data.match_type_) {
  case Http::HeaderUtility::HeaderMatchType::Value:
    return config_data.value_.empty() || request_data.value_ == config_data.value_;
  case Http::HeaderUtility::HeaderMatchType::Range: {
    int64_t value = 0;
    return StringUtil::atol(request_data.value_.c_str(), value, 10) &&
...
RouteConstSharedPtr ParamterRouteEntryImpl::matches(const MessageMetadata& metadata,
  ...
  for (auto& config_data : paramter_data_list_) {
    const auto* data = metadata.getParameterValue(config_data.index_);
    if (data == nullptr) {
      ENVOY_LOG(debug,
...
RouteConstSharedPtr MethodRouteEntryImpl::matches(const MessageMetadata& metadata,
  ...
  if (is_contain_wildcard_) {
    if (!Utility::wildcardMatch(metadata.method_name().c_str(), method_name_.c_str())) {
      ENVOY_LOG(
          debug,
          "dubbo route matcher: method matching failed, origin method '{}', input method '{}'",
          method_name_, metadata.method_name());
      return nullptr;
    }
  } else {
...
RouteConstSharedPtr RouteMatcher::route(const MessageMetadata& metadata,
  ...
  if (interface_name_ == metadata.service_name() &&
      (group_.value().empty() ||
       (metadata.hasServiceGroup() && metadata.service_group() == group_)) &&
      (version_.value().empty() ||
       (metadata.hasServiceVersion() && metadata.service_version() == version_))) {
